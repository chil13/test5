...
public class QuestionDAO {
	...
	public ArrayList<QuestionBean> test(int q_id) throws SQLException {
		...
		try {
			// ステートメントの作成
			String sql = "SELECT q_id,question,intention FROM question WHERE q_id = ? ";
	...
	public ArrayList<QuestionBean> questionById(ArrayList<RetryBean> retryList) throws SQLException {
		...
		try {
			...
			String sql = "SELECT q_id,question,intention FROM question WHERE q_id IN " + listStr;
	...
	public ArrayList<QuestionBean> reQuestion(String reQuestion) throws SQLException {
		ArrayList<QuestionBean> list = new ArrayList<QuestionBean>();
		...
		try {
			// ステートメントの作成
			String sql = "SELECT q_id FROM question WHERE question = ? ";
			preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, reQuestion);
